...
def get_current_user(token: str = Depends(outh2_scheme), db: Session = Depends(database.get_db)):  # Once the verify access token function returns the user ID (token data), then this fetches the user from the database
